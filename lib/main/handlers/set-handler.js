   ...
   async handle(req) {
    ...
    async CreateSet(userID) {
        ...
        try {
          ...
          const validMetrics = enumList.MetricEnum;
          const validUnit = enumList.UnitMetricEnum;
          ...
          if (req_body.metric && !validMetrics.includes(req_body.metric)) {
            throw new ValidationException("Bad request: Invalid metric value");
          }
          
          if (req_body.unit && !validUnit.includes(req_body.unit)) {
            throw new ValidationException("Bad request: Invalid unit");
          }
          ...
          const connectionDB = await db.connection;
          var checkQuery = 'SELECT * FROM sets WHERE exerciseID = ? AND userID = ? AND workoutID = ? AND Date = ? ';
          var getPromise = new Promise ((resolve, reject) => {
            connectionDB.query(checkQuery,[req_body.exerciseID, userID, req_body.workoutID, req_body.Date],
            function (err, result) {
    ...
    async UpdateSet(userID, setID) {
        ...
        try {
            ...
            if (req_body.metric) {
                if (!validMetrics.includes(req_body.metric)) {
                    throw new ValidationException("Bad request: Invalid metric value");
                } else {
                    updates.push(`metric = '${req_body.metric}'`);
                }
            }
            if (req_body.unit) {
                if (!validUnit.includes(req_body.unit)) {
                    throw new ValidationException("Bad request: Invalid unit value");
                } else {
                    updates.push(`unit = '${req_body.unit}'`);
                }
            }
