...
class ExerciseHandler extends Handler {
	...
	async CreateExercise(req) {
		...
		await isValidEnumValue("progressionEnum", req_body.progression,true);
		await isValidEnumValue("forceEnum", req_body.forces,true);
		await isValidTimeFormat(req_body.rest_interval);
		await isValidEnumValue("targetMuscleEnum", req_body.target_muscle_group,true);
...
async ListExercise(req) {
	...
	async UpdateExercise(req) {
		...
		try {
			...
			await isValidEnumValue("progressionEnum", req_body.progression,false);
			await isValidEnumValue("forceEnum", req_body.forces,false);
			await isValidTimeFormat(req_body.rest_interval);
			await isValidEnumValue("targetMuscleEnum", req_body.target_muscle_group,false);
