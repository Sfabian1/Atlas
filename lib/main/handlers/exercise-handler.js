...
class ExerciseHandler extends Handler {
	...
	async CreateExercise(userID) {
		try {   
			...
			const validRestIntervalMetrics = enumList.restIntervalMetricEnum;
			...
			if (!isValidTimeFormat(req_body.rest_interval)) {
			  ...
			  if (req_body.rest_interval_metric && !validRestIntervalMetrics.includes(req_body.rest_interval_metric)) {
				throw new ValidationException("Bad request: Invalid rest_interval_metric");
			  }  
			  ...
			  exerciseID = String(exerciseID).slice(0, 17);
			  exerciseID = BigInt(exerciseID);
			  const query = "INSERT INTO exercise VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
			  connectionDB.query(query, [exerciseID, userID, req_body.name, req_body.target_muscle_group, req_body.forces, req_body.rest_interval, req_body.progression, req_body.link, req_body.rest_interval_metric]);
