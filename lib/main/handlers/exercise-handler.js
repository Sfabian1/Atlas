...
class ExerciseHandler extends Handler {
	...
	async ListExercises(req) {
		try {
			...
			return await listPromise.then(resp => {
				const query = `SELECT * FROM defaultexercise`;
				connectionDB.query(query, function (err, result) {
					if (err) 
						reject (new InternalServerException(err));
					...
					if (result == null || result.length == 0){
						...
						console.log("Unable to find any default exercises");
						reject(new ResourceNotFoundException(`Unable to any default exercises`));
						...
						var string= JSON.stringify(result+resp);
						console.log('>> string: ', string );
						resolve(new CompletedResponse(string, "applicaiton/json"));			
					}
				 });
				return resp;
			}).catch(err => {
	...
	async GetExercise(req) {
		try {
            ...
            const connectionDB = await db.connection;
            const query = `SELECT * FROM exercise WHERE user_id = '${userID}' AND exercise_id = '${exerciseID}' LIMIT 1`;
