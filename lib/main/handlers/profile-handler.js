...
class ProfileHandler extends Handler {
            ...
            switch (req.method) {
                ...
                case "GET":
                    if (req.url.split('/').length == 4) {
                        ...
                        var resp = this.ListProfile(getUserID(req.url));
                        console.log(resp);
					    return resp;
                    }
                case "PUT":
                ...
                connectionDB.query(query, (err, result) => {
                    ...
                    if (result && result.length) {
                       ...
                       if (result.length === 0) {
                          ...
                          resolve(new CompletedResponse(String(profileData), 'application/json'));
                       }
                    } else {
    ...
    async createProfile(userID) {
        try {
            ...
            var profileID = generateShortUUID();
            ...
            resolve( new CompletedResponse(String(profileID), 'text/plain'));

            //return new CompletedResponse(`Profile created successfully', 'text/plain', 201);
            }
                });
            });
            return await getPromise.then(resp => {
