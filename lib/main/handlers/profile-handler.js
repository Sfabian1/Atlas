...
class ProfileHandler extends Handler {
    ...
    async createProfile(req) {
        try {
            ...
            const connectionDB = await db.connection;
            var checkQuery = 'SELECT * FROM profile WHERE user_id = ? AND username = ? AND height = ? AND weight = ? AND bmi = ? AND age = ?  ';
            var getPromise = new Promise ((resolve, reject) => {
                connectionDB.query(checkQuery, [userID, req_body.username, req_body.height, req_body.weight, req_body.bmi, req_body.age], function (err, result) {
                        if (err)
                        reject (new InternalServerException(err));
                        ...
                        if (resultFinal.length > 2){
                            reject( new ConflictException("Match found :(,repeated values"));
